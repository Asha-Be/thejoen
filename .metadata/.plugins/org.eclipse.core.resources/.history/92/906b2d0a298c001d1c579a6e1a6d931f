<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8" import="mysql.*" import="java.util.*"%>
...
<%
	...
	int starRow = (curentPage - 1) * pageSize + 1;
	//(1-1) * 10 + 1 = 1
	//(2-1) *10 +1 = 11
	//(3-1) *10 +1 = 21  페이지넘어가는 기준일라나

	int endRow = curentPage * pageSize; // 1*10 = 10, 2*10 = 20....
											// 2*10 == 10 > 2페이지 끝행
											//3* 10 == 30 >3 페이지 끝행..
	...
	number = count - (curentPage - 1) * pageSize;
	//count=17
	...
	<table border="1">
		...
		<tr>
			...
			<td>
			 	...
			 	int wid = 0;
							if (dto.getLevelNo() > 0) {//답글이 있는가? 있다고한다면
				 				wid = 5 * (dto.getLevelNo()); //5의 배수로커진다네
				 ...
				 %> <img src="../imgs/level.gif" width="<%=wid%>" height="16">
								...
								href="content.jsp?num=<%=dto.getNum()%>&pageNum=<%=curentPage%>">
									<%=dto.getTitle()%>
			...
			<td align="center">
				<%
					//전체 페이지 수 구하기
						...
						int startPage = (int) (curentPage / pageBlock) * 10 + 1; //시작 페이지 구하기
